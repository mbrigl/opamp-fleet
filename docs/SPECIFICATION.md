...
The control plane is closed to anyone who cannot prove they belong to the fleet: every connection is
encrypted and authenticated, so configuration and software alike travel a trusted channel. The Server
itself carries no user interface: it exposes a stable REST API and lets any portal render the fleet
however it likes. The client is one process that can supervise many agents at once, and
growing to a new kind of agent means writing a new plugin, not changing the core. The project grows
...
- **Headless Server, decoupled UI.** The Server's public contract is a **REST API**; it ships no
  mandatory UI of its own. A UI lives wherever the operator wants it — a standalone app or an existing
  portal — and drives the fleet entirely through that API.
...
- **Authenticate and encrypt the control plane.** Every connection the Server accepts is TLS-encrypted
  and must present a shared token, so the fleet's control plane is not open to anyone who can reach the
  network. This is the trust boundary that configuration distribution and software updates ride on, not
...
- **Develop against real agents.** The development environment runs actual agents next to the Server —
  the upstream OpenTelemetry OpAMP Supervisor and Collector, and third-party collectors — so protocol
  compatibility is exercised against the real thing rather than a mock.
...
- **Server** — this project's control plane. It tells Agents which configuration they should be running
  and records what they report back. There is exactly one Server per fleet. It is **headless**: its
  only interface is the REST API.
- **REST API** — the Server's stable, machine-readable HTTP interface for reading fleet state and
  reading and changing configuration. It is the integration contract any UI or portal builds on.
...
- **Supervisor Host** — the client process. A single Supervisor Host runs **multiple Supervisor
  instances** at once, loading each as a plugin. It is the one deployable that a machine runs to place
  its agents under management.
...
- **Instance UID** — the identity of an Agent instance, stable across its restarts.
...
- **Shared token** — an out-of-band secret the Server and its Agents both hold and present on every
  connection, so the Server can authenticate that a peer belongs to the fleet. It is a single fleet
  secret, not a per-Agent identity. Carried over a TLS-encrypted transport, it is the trust boundary
  that configuration distribution and software updates ride on.
...
5. **Any UI can drive the fleet.** The REST API exposes fleet state and configuration as a stable
   contract; an external UI reads the fleet and changes what it runs entirely through that API, with no
   UI built into the Server.
...
10. **The Server updates an Agent's software, not only its configuration.** Via OpAMP package delivery
    ...
    reported, not silent.
11. **The transport is authenticated and encrypted.** The Server requires a TLS-encrypted connection
    and a valid shared token on every Agent connection — and on package downloads — refusing anything
    that cannot present it, so neither configuration nor software distribution rides on an open channel.
12. **The environment proves it.** A developer can bring up the Server together with real agents —
    the upstream OpenTelemetry Supervisor and Collector and at least one third-party collector — and
    observe the functional criteria above end to end, confirming protocol compatibility against real
    implementations.
...
- **Shipping a production UI.** The Server provides the REST API; user interfaces are external and out
  of scope for this project to build.
- **Authorization and multi-tenancy.** Beyond authenticating that a peer belongs to the fleet, the
  Server does not distinguish *which* Agent may do *what*, nor separate one operator's fleet from
  another's. Per-Agent identity, roles, and tenancy are real needs deferred rather than half-built.
