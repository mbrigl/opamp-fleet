...
| `OffersPackages` | `0x0008` | Beta | optional | implemented | Declared only while a non-empty package store is armed (`packages_dir`, ADR-0015). Artifacts and metadata persist and are managed through the REST API — uploaded under `max_package_size_bytes` and served straight from disk, so neither the store nor a download holds a program in memory; the hash-gated `PackagesAvailable` offer carries a `download_url` served from the same listener. |
