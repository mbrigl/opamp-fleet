...
| `AcceptsPackages` | `0x0008` | Beta | optional | implemented | Software distribution for Managed Processes (goal 10, ADR-0015). Declared by a Supervisor-backed Agent whose block names its program with a bare file name, which puts it in a directory this Client owns and is therefore the whole of its consent (ADR-0021); the Server chooses which artifact (ADR-0017); the offered artifact is streamed to disk, verified (content hash always, Ed25519 signature when a key is configured), unpacked when it is a `.tar.gz` or a `.7z` — the member named after the Managed Process's binary, since an upstream release ships an archive and nothing repacks it on the way (ADR-0018); the `DownloadableFile.headers` the offer names ride the `GET`, which is what makes a **referenced** artifact behind an authenticated mirror reachable at all (ADR-0018) — and because such a header is an operator's credential for *one* host, a download that carries any follows its redirect chain by hand and re-attaches them only while scheme, host and port are unchanged, since `reqwest` strips only `Authorization`, `Cookie` and `Proxy-Authorization` across origins and a custom token would otherwise be handed to wherever a mirror points; a header the offer names that is not a valid HTTP header fails the download by *key*, never printing its value; an encrypted `.7z` opens with `[packages] archive_key`, which the Server never learns — swapped over that binary by the Supervisor inside `<supervisor_dir>/<name>/program/`, health-gated on `apply_grace_secs`, and rolled back if it will not stay up. A Managed Process named by an *absolute* path is the machine's, declares no package capability, and is never written to. One that is **more than a single file** — an executable plus the shared objects it loads — is delivered by naming `program_path` in its block (ADR-0023): the whole archive is then unpacked into `<supervisor_dir>/<name>/program/tree/`, the tree it replaced kept beside it and put back whole if the new one will not stay up. Since the archive then names paths, every member is validated before anything is written and one this Client will not write — a `..` or absolute path, a symbolic or hard link, more than 10 000 members, more than 2 GiB unpacked — refuses the whole archive. Only a `TopLevel` package is installed: an `Addon` is what a Supervisor has no way to apply, so it is refused with `InstallFailed` rather than written over the binary it was meant to extend. **The Client updates itself the same way** (goal 11, ADR-0020): it is always its own Agent, and with `[self_update]` in `supervisor.toml` it accepts the one package that section names — anything else is refused, since a package with an empty Selector reaches every consenting Agent and one written over the Client takes the host out of reach. The artifact is staged as a new version *beside* the running one in the ADR-0010 layout, proved by running `supervisor self-check` on it before the `current` pointer moves, and the process then exits asking the service manager for a restart. A marker in the state directory carries the outcome across that restart: the new version commits itself once it reaches the Server, and a version that will not stay up is rolled back to its predecessor after three attempts. |
| `ReportsPackageStatuses` | `0x0010` | Beta | optional | implemented | `Downloading` while the artifact is on the wire, `Installing` once the Supervisor applies it, then `Installed`/`InstallFailed` (ADR-0015), with the offered `all_packages_hash` echoed once terminal — which is what stops the Server re-offering. The `Downloading` reports repeat every 5 s and carry `PackageDownloadDetails` (percent, bytes per second), so a transfer of hundreds of megabytes stays distinguishable from a stuck install; both that status and the details are `[Development]` upstream. Survives restarts via the persisted installed-package record. Withheld from a Server that has declared capabilities without `AcceptsPackagesStatus` (ADR-0087) — a suppressed status is not queued, it returns with the full snapshot that follows any reconnect or `ReportFullState`. |
| `ReportsOwnTraces` | `0x0020` | Beta | optional | implemented | **Five fleet operations are traces** ([ADR-0090](adr/0090-own-traces-come-from-the-clients-own-tracing-spans.md)): installing a package, applying a configuration — a Managed Process's, and the Supervisor set — applying offered connection settings, and this Client's own self-update. Each is a root span whose children are its phases and whose status is the outcome the Server is told, so a failed rollout is one trace naming the phase that failed rather than a log hunt. They are the `tracing` spans this Client already writes, converted by `tracing-opentelemetry`; the appender that carries the logs converts events and not spans, which is why the exporter had nothing to export until that layer existed. A self-update stays **one** trace across the restart it needs: the trace rides in the update marker, so the commit or the rollback a later process performs continues what an earlier one began. What is deliberately *not* traced is message handling — a transport exchange, a sampler tick, a Supervisor Endpoint message — which the reference `opampsupervisor` does span; they have no outcome and no end, and at one exchange per Agent per interval they would bury the five operations. Log records written inside an operation carry its `TraceId`, which is what makes a log-to-trace join answerable at the receiving end. Exported OTLP/HTTP with protobuf bodies to the destination the Server names in `ConnectionSettingsOffers.own_*` (ADR-0036) — never to one configured locally, which is what the capability means. Declared unconditionally: it states an ability, and gating it on a destination already in force would mean the Server could never make the first offer. A destination on plain `http://` beyond loopback is **refused and reported**, the Baseline's "MAY refuse" taken; so are its `tls` and `proxy` fields, for the reasons the OpAMP settings row gives. An offered `certificate` **is** honoured (ADR-0036): its `cert` is presented by the OTLP exporter, paired with the key this Client generated for its CSR — the ADR-0035 machinery reused as-is. A `private_key` *in the offer* is refused by name, because this Client's private key never leaves its host and is never accepted from the Server; `ca_cert` is not added to the trust store, which is the Baseline's own advice ("It is not recommended that the Agent accepts this CA as an authority for any purposes"). And "reported" now means reported: a refusal reaches the Server as a `FAILED` `connection_settings_status` naming it, including one found at startup when the persisted settings are put back in force — where saying nothing would have the Client claim on every reconnect that a destination is running which it refused to use. The OTLP Resource carries the Agent's identifying attributes, as the Baseline asks. The exporters are handed an HTTP client bound to this process's Tokio runtime: the SDK exports from a dedicated thread that blocks on the request and has no reactor of its own, so an asynchronous client used there panics rather than exports; that client also bounds the export, which neither the SDK's periodic reader nor `opentelemetry-otlp` does for a client it was handed — without the bound, a destination that stops answering rather than refusing held the exporter thread until the Client restarted. **A destination can also be taken away** ([ADR-0089](adr/0089-an-own-telemetry-offer-states-all-three-destinations.md), a recorded [deviation](#deviations)): an offer naming any of the three signals states all three, so one it leaves out is stopped, and an endpoint offered empty withdraws that signal — shut down, dropped from the persisted settings, and acknowledged `APPLIED`, since "stop" is not a refusal to report back. |
| `ReportsOwnMetrics` | `0x0040` | Beta | optional | implemented | Process metrics per Agent, sampled from the outside: the Client's own process for its own Agent, the Managed Process for a Supervisor-backed one, named after the (experimental) process semantic conventions. Sampled **and exported every 10 s**, the Baseline's *"recommended reporting interval"* — one constant drives both, since exporting more often than sampling would ship each value repeatedly and turn a gauge series into a step function. The SDK's periodic reader defaults to 60 s, so the interval is set explicitly rather than inherited; that default is how it silently sat at six times the recommendation. Exported OTLP/HTTP with protobuf bodies to the destination the Server names in `ConnectionSettingsOffers.own_*` (ADR-0036) — never to one configured locally, which is what the capability means. Declared unconditionally: it states an ability, and gating it on a destination already in force would mean the Server could never make the first offer. A destination on plain `http://` beyond loopback is **refused and reported**, the Baseline's "MAY refuse" taken; so are its `tls` and `proxy` fields, for the reasons the OpAMP settings row gives. An offered `certificate` **is** honoured (ADR-0036): its `cert` is presented by the OTLP exporter, paired with the key this Client generated for its CSR — the ADR-0035 machinery reused as-is. A `private_key` *in the offer* is refused by name, because this Client's private key never leaves its host and is never accepted from the Server; `ca_cert` is not added to the trust store, which is the Baseline's own advice ("It is not recommended that the Agent accepts this CA as an authority for any purposes"). And "reported" now means reported: a refusal reaches the Server as a `FAILED` `connection_settings_status` naming it, including one found at startup when the persisted settings are put back in force — where saying nothing would have the Client claim on every reconnect that a destination is running which it refused to use. The OTLP Resource carries the Agent's identifying attributes, as the Baseline asks. The exporters are handed an HTTP client bound to this process's Tokio runtime: the SDK exports from a dedicated thread that blocks on the request and has no reactor of its own, so an asynchronous client used there panics rather than exports; that client also bounds the export, which neither the SDK's periodic reader nor `opentelemetry-otlp` does for a client it was handed — without the bound, a destination that stops answering rather than refusing held the exporter thread until the Client restarted. **A destination can also be taken away** ([ADR-0089](adr/0089-an-own-telemetry-offer-states-all-three-destinations.md), a recorded [deviation](#deviations)): an offer naming any of the three signals states all three, so one it leaves out is stopped, and an endpoint offered empty withdraws that signal — shut down, dropped from the persisted settings, and acknowledged `APPLIED`, since "stop" is not a refusal to report back. |
...
| `AcceptsOpAMPConnectionSettings` | `0x0100` | Beta | optional | partial | Server-driven credential rotation (goal 17, ADR-0014) and client-certificate rotation (ADR-0035). An offer carrying OpAMP settings is verified by actually connecting (the Baseline's MUST, which it scopes to `ConnectionSettingsOffers.opamp` alone), persisted in the Client's state dir (overriding `supervisor.toml`), then the connection switches — across transports if the offered endpoint demands it. An offer carrying **only** telemetry destinations is a different class and takes a different path (ADR-0086): the Baseline names three classes of destination with deliberately different sequences, so such an offer is applied in place and acknowledged **without** a verification connection and **without** a reconnect — there is nothing to prove by connecting to the OpAMP endpoint, and a receiver that is momentarily down is not an offer that is wrong. Before that it was dropped outright, which left this project's own telemetry-only Server re-offering for ever. What is persisted says only what was offered: no synthetic empty `opamp` block. The two halves fold differently and deliberately (ADR-0089): the `opamp` settings carry only what changes, while the telemetry destinations are stated as a whole by any offer that mentions them. An offered `heartbeat_interval_seconds` becomes the heartbeat (WebSocket) or polling interval (plain HTTP). An offered `certificate` is proved by connecting **with it** before it replaces the one in force, and is then presented on both transports. **Two fields are still not honoured**: `tls` and `proxy` (both `[Development]`) are dropped — but the offer is no longer acknowledged `APPLIED` when they are: the Client applies everything else and reports `FAILED` with an `error_message` naming what it dropped, so a Server learns of the gap instead of being told success. `TLSConnectionSettings` is refused on merit — most of what it can say weakens verification, and `insecure_skip_verify` would let a Server disarm the check that proves it is the Server (ADR-0035). |
...
| `ReportsConnectionSettingsStatus` | `0x8000` | Development | optional | implemented | `APPLYING` on receipt, `APPLIED`/`FAILED` after applying, the hash echoed either way (ADR-0014) — which is what stops the Server re-offering. **One acknowledgement per offer**, whose `error_message` names everything dropped across both halves of it (ADR-0086 clause 3): the Baseline hashes all settings together, so the Agent answers the message, never one part of it. Survives restarts via the persisted settings. Gated on the Server's `OffersConnectionSettings` — *unless* that Server has actually sent an offer, which arms the report whatever its bitmask says (ADR-0087 clause 2); without that exception a Server offering without declaring could never stop offering. |
...
| `OffersPackages` | `0x0008` | Beta | optional | implemented | Declared only while a non-empty package store is armed (`packages_dir`, ADR-0015). The store holds **Packages** (ADR-0095): each identified by *(Agent type, version)* and nothing else — no name of its own, no Selector, no kind flag — holding one entry per platform, an uploaded artifact or a reference (ADR-0018), persisted and managed through the REST API, uploaded under `max_package_size_bytes` and served straight from disk, so neither the store nor a download holds a program in memory. Beside them, in the same directory, live **Deployments** (ADR-0096): a name, the channel a Selector aims at, at most one Package per Agent type, and each artifact's Ed25519 signature. A Deployment is the only thing rolled out. The offer is composed **per Agent** from what an explicit rollout act assigned to it (ADR-0061) — never from matching, which only computes what such an act *would* release — as the Baseline's "available on the Server for this Agent" describes, and its `all_packages_hash` is computed over that same assignment. **An Agent belongs to at most one Deployment**: where two claim it, that is a conflict, it is proposed nothing, and the fleet view names both — there is no specificity ranking and no version tie-break left to decide between them (ADR-0017 withdrawn by ADR-0096). Fit runs first and is two mandatory steps: a Package built for another **Agent type** is dropped — the type in its identity is matched against the `service.name` the Agent reports (ADR-0033, ADR-0034), and an Agent reporting none fits nothing — and then every Package without an entry for the Agent's **platform** (ADR-0031). What survives must also be an **upgrade** over what the Agent reports running (ADR-0083). An uploaded entry's `download_url` points at the Agent plane's listener — the one the Client is already connected to, which is why the path resolves against its own endpoint (ADR-0066); a **referenced** one carries the address, the operator's SHA-256, and any headers the source needs, verbatim — the Baseline's Download Server *"may be on the same host as the OpAMP Server or a different host"*. The `signature` an offer carries is the **assigned Deployment's** for those exact bytes on that platform, not a property of the artifact record; a channel holding none offers the artifact unsigned, which the Server reports rather than refuses (ADR-0015). Only `TopLevel` is ever offered: an Agent has one binary to replace and its Deployment holds one Package for its type, so the Baseline's "normally only one top-level package" is structural here rather than a flag anyone can set (ADR-0095). Taking a version back is **not** a rollout: the fleet only moves forward, and what reverses a bad release is the host's own health-gated rollback (ADR-0058). |
...
| Message size limits | Both ends MUST enforce a receive limit and MUST NOT send past it; `413` on HTTP, close `1009` on WebSocket | implemented | New in `v0.19.0` — see [Message size limits](#message-size-limits). Default 64 MiB, the upstream recommendation; `max_message_size_bytes` in `server.toml` and `supervisor.toml` tightens it (ADR-0008). |
...
| Deviation | Reason |
|---|---|
| **An own-telemetry offer states all three destinations, and an empty `destination_endpoint` withdraws one** ([ADR-0089](adr/0089-an-own-telemetry-offer-states-all-three-destinations.md)) | Departs from two sentences of the schema. `ConnectionSettingsOffers.own_metrics` (and `own_traces`, `own_logs`) says *"If this field is not set then the Agent should assume that the settings are unchanged"*; here that holds for an offer **silent about telemetry**, while an offer that names any of the three states all three, so a signal it leaves out is stopped. And `TelemetryConnectionSettings.destination_endpoint` says *"The value MUST be a full URL an OTLP/HTTP/Protobuf receiver with path"*; an empty value is sent and honoured as an explicit withdrawal. **Why:** taken literally, the delta reading leaves own telemetry impossible to switch off from the fleet — nothing in the Baseline defines a withdrawal for these fields, the way `certificate` defines one for itself — and it disagrees with the only implementation that has these fields: `opampsupervisor` builds its own-telemetry section from the received message alone (`updateOwnTelemetryData` drops a `nil` or empty endpoint; `setupOwnTelemetry` logs *"Disabling own telemetry pipeline in the config"* when nothing is left), with no fold against what it stored. Following the schema here would mean a Server of ours silently disabling a supervisor's other signals, and a supervisor-shaped Server unable to disable ours. The two cannot both be right, and settling that is upstream's to do: this project takes no position in `opamp-spec` and records the departure here instead. |
| *(nothing else)* | Two SHOULDs are consciously not taken up yet and noted in the matrix: response compression on plain HTTP, and Server-side throttling. One MUST is *conditional* and currently unmet rather than departed from — the `instance_uid` check on a CSR that carries one, which no Client in this project produces; it is stated under [Mutual TLS](#mutual-tls-and-the-two-fields-still-refused) and tracked as a hardening measure, not as a deviation. |
