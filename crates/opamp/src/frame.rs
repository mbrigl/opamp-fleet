...
/// The largest message we accept on either transport, framing header included. The Baseline leaves
/// the limit to the implementation (upstream later recommends 64 MiB); agents send small status
/// reports, so a generous cap turns away only garbage and runaway payloads, never a legitimate
/// report.
